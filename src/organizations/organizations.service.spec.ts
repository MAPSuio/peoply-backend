...
describe("OrganizationsService", () => {
  ...
  describe("MAPS moderation privileges", () => {
    /* The role list handed to checkUserRole is the whole authorisation
       decision, so assert on it rather than on the boolean it happens to
       return for one mocked row. */
    const rolesAskedFor = () =>
      prisma.userOrganizationRole.findFirst.mock.calls[0][0].where.role.in;

    it("lets any MAPS member read the moderation queue", async () => {
      prisma.userOrganizationRole.findFirst.mockResolvedValueOnce({
        role: "MEMBER",
      });

      await expect(service.ensureMapsMember("user-1")).resolves.toBeUndefined();
      expect(rolesAskedFor()).toEqual(["MEMBER", "ADMIN", "OWNER"]);
    });

    it("does not accept a plain MAPS member for approval changes", async () => {
      // Unapproving hides an organization and all its events platform wide.
      // MEMBER is the rung every invited volunteer lands on, and it does not
      // even carry the right to edit their own organization.
      prisma.userOrganizationRole.findFirst.mockResolvedValueOnce(null);

      await expect(service.ensureMapsAdmin("user-1")).rejects.toThrow(
        "You must be a MAPS admin to change approval",
      );
      expect(rolesAskedFor()).toEqual(["ADMIN", "OWNER"]);
      expect(rolesAskedFor()).not.toContain("MEMBER");
    });

    it("lets a MAPS admin change approval", async () => {
      prisma.userOrganizationRole.findFirst.mockResolvedValueOnce({
        role: "ADMIN",
      });

      await expect(service.ensureMapsAdmin("user-1")).resolves.toBeUndefined();
    });

    it("scopes both checks to the MAPS organization", async () => {
      // Being an admin of your own organization must not grant moderation.
      prisma.userOrganizationRole.findFirst.mockResolvedValueOnce(null);

      await expect(service.ensureMapsAdmin("user-1")).rejects.toThrow();
      expect(
        prisma.userOrganizationRole.findFirst.mock.calls[0][0].where
          .organizationId,
      ).toBe("c997beea-620f-4b83-bb97-12f3c0b96a14");
    });
  });
});
